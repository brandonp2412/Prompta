...
def test_cache_migration_repairs_unambiguous_orphaned_transient_tools(
    tmp_path: Path,
) -> None:
    path = tmp_path / "chats.sqlite3"
    conversation_id = "conversation-orphan-repair"
    cache = ChatCache(path)
    cache.start(
        conversation_id,
        context_id="context-orphan-repair",
        job_name="",
        prompt="Do work",
    )
    cache.write_snapshot(
    ...
    transient_key = "__prompta_live_assistant_orphan_repair__"
    source_events = [
        {
            "id": "call-1",
            "role": "assistant",
    ...
    repaired = ChatCache(path)
    calls = repaired.connection.execute(
        """
        SELECT message_key, connector, action, status
        FROM tool_calls
        WHERE conversation_id = ?
        """,
        (conversation_id,),
    ).fetchall()
    stale = repaired.connection.execute(
        """
        SELECT COUNT(*)
        FROM tool_calls
        WHERE conversation_id = ? AND message_key = ?
...
def test_cache_migration_leaves_ambiguous_orphaned_transient_tools_untouched(
    tmp_path: Path,
) -> None:
    path = tmp_path / "chats.sqlite3"
    conversation_id = "conversation-orphan-ambiguous"
    cache = ChatCache(path)
    cache.start(
        conversation_id,
        context_id="context-orphan-ambiguous",
        job_name="",
        prompt="First",
    )
    cache.write_snapshot(
        conversation_id,
        {
            ...
            "messages": [
                ...
                {"id": "a2", "role": "assistant", "content": "Second done"},
            ],
            "streaming": False,
        },
        complete=True,
    ...
    transient_key = "__prompta_live_assistant_orphan_ambiguous__"
    raw = sqlite3.connect(path)
    raw.row_factory = sqlite3.Row
    persist_structured_capture(
        raw,
        ...
        source_events=[
            {
                ...
                "content_type": "code",
                "text": json.dumps(
                    {"path": "/Test MCP/link_123/inspect", "args": {"target": "kite"}}
                ),
                "create_time": 2.0,
            }
        ],
    ...
    calls = reopened.connection.execute(
        """
        SELECT message_key
        FROM tool_calls
        WHERE conversation_id = ?
    ...
    assert [row["message_key"] for row in calls] == [transient_key]
