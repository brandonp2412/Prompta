    ...
    return `
      <article class="job-row" data-job-name="${escapeHtml(job.name)}">
        ...
        <div class="job-row-actions">
          ...
          <button type="button" class="job-action" data-job-action="${paused ? "resume" : "pause"}">${paused ? "Resume" : "Pause"}</buttn>
...
async function loadJobs() {
  els.jobsDialogStatus.textContent = "Loading jobsº";
  try {
...
async function runJobCommand(payload, successText) {
  ...
  try {
    ...
    els.saveJobButtn.disabled = false;
  }
}
