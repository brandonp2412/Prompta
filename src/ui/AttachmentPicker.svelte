<script lang="ts">
  ...
  let photoClickRequest = $state(0);
  let cameraClickRequest = $state(0);
  ...
  function truncate(value: string, length = 28) {
    return value.length > length
      ? value.slice(0, Math.max(1, length - 1)).trimEnd() + "…"
      : value;
  }
  ...
  function select(kind: "file" | "photo" | "camera") {
    ...
    if (kind === "file") fileClickRequest += 1;
    else if (kind === "photo") photoClickRequest += 1;
    else cameraClickRequest += 1;
  }
  ...
  export function configure(next: AttachmentPickerOptions) {
    options = next;
  }
  ...
  export function closeMenu(restoreFocus = false) {
    menuOpen = false;
    if (restoreFocus) pickerFocusRequest += 1;
  }
...
<div class="composer-tools" {@attach clickOutside(() => (menuOpen = false))}>
  <button
    {@attach focusOnRequest(() => pickerFocusRequest)}
    type="button"
  ...
  />
  <input
    {@attach clickOnRequest(() => photoClickRequest)}
    id="photoUploadInput"
    type="file"
    accept="image/*"
    hidden
  ...
  />
  <input
    {@attach clickOnRequest(() => cameraClickRequest)}
