...
var attachmentPicker$1, changelogDialog, logsPanel$1;
...
function matchingPendingReplyMessageIndex(messages, pending, claimedIndexes = /* @__PURE__ */ new Set()) {
	...
	const attachmentOnly = !content && Boolean(pending.attachmentNames?.length || pending.attachments?.length);
	if (!content && !attachmentOnly) return -1;
	const matchesPendingContent = (message) => {
	...
	if (pending.origin === "new") {
		const firstDurableUserIndex = messages.findIndex((message, index) => !claimedIndexes.has(index) && matchesPendingContent(message));
...
function pendingSendPreviewText(message, attachmentNames = []) {
...
function messageDisplayContent(message) {
	...
	const liveToolParts = Array.isArray(message.parts) ? message.parts.map((part, index) => ({
	...
	})).filter((part) => part.kind === "tool_call" && Boolean(part.content)).sort((left, right) => left.ordinal - right.ordinal || left.index - right.index) : [];
	let liveToolIndex = 0;
	const enrichedFallback = fallback.replace(LIVE_GENERIC_TOOL_PLACEHOLDER, (placeholder) => {
		...
		liveToolIndex += 1;
		return part.toolCallKey && toolDiffs.has(part.toolCallKey) ? injectToolCodeDiff(part.content, toolDiffs.get(part.toolCallKey)) : part.content;
	});
	...
	return ordered.map((part) => part.content).join("\n\n").trim() || enrichedFallback;
}
var LIVE_GENERIC_TOOL_PLACEHOLDER;
var init_conversationLogic = __esmMin((() => {
	init_diffPreview();
	LIVE_GENERIC_TOOL_PLACEHOLDER = /```(?:tool|tool-call|function|function-call):\s*tool[^\n]*\n\s*Called tool\s*\n```/gi;
}));
//#endregion
...
var root_10$1 = /* @__PURE__ */ from_html(`<div role="presentation"><!> <!></div> <dialog class="pending-message-actions" aria-labelledby="pendingMessageActionsTitle"><div class="pending-message-actions-shell"><div id="pendingMessageActionsTitle" class="pending-message-actions-title">Pending message</div> <!> <button type="button" class="pending-message-action"><svg viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="m4 20 4.2-1 10.9-10.9a2.1 2.1 0 0 0-3-3L5.2 16 4 20Zm10.6-13.4 3 3"></path></svg> <span>Edit message</span></button> <button type="button" class="pending-message-action danger"><svg viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="M4 7h16M9 7V4h6v3m3 0-1 13H7L6 7m4 4v5m4-5v5"></path></svg> <span>Delete message</span></button> <button type="button" class="pending-message-action cancel"><svg viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="M6 6l12 12M18 6 6 18"></path></svg> <span>Cancel</span></button></div></dialog>`, 1);
...
init_browserAttachments_svelte();
init_clientLogic();
var root$4 = /* @__PURE__ */ from_html(`<div class="jobs-empty">No scheduled jobs.</div>`);
...
var root_10 = /* @__PURE__ */ from_html(`<section class="jobs-page" aria-labelledby="jobsPageTitle"><div class="jobs-page-shell"><header class="jobs-page-header"><div class="chat-heading"><div class="heading-title" id="jobsPageTitle">Scheduled jobs</div> <div class="heading-meta">Create and manage scheduled prompts.</div></div></header> <div class="jobs-page-status" role="status"> </div> <div class="jobs-list"><!> <!></div> <!> <form class="jobs-form"><h3> </h3> <label><span>Name</span> <input id="jobName" name="name" autocomplete="off" required=""/></label> <label><span>Prompt</span> <textarea id="jobPrompt" name="prompt" rows="3" required=""></textarea></label> <div class="jobs-form-grid"><label><span>Schedule</span> <select id="jobSchedule" name="schedule"><option>Interval</option><option>Daily</option></select></label> <!></div> <!> <div class="jobs-form-actions"><button type="button" class="jobs-secondary-button"> </button> <button type="submit" class="jobs-primary-button">Save job</button></div></form> <div class="jobs-page-footer"><button type="button" class="jobs-danger-button">Clear all jobs</button></div></div> <dialog class="jobs-confirm-dialog" aria-labelledby="jobsConfirmTitle" aria-describedby="jobsConfirmDescription"><div class="jobs-confirm-shell"><h3 id="jobsConfirmTitle"> </h3> <p id="jobsConfirmDescription"><!></p> <div class="jobs-confirm-actions"><button type="button" class="jobs-secondary-button">Cancel</button> <button type="button" class="jobs-danger-button"> </button></div></div></dialog></section>`);
function JobsPage($$anchor, $$props) {
	...
	let confirmation = /* @__PURE__ */ state$1(null);
	...
	async function confirmDestructiveAction() {
		...
		if (await command({
			...
			if (get(editing) === job.name) reset$1();
		}
	}
	user_effect(() => {
		reset$1();
	...
	var text$1 = only_child(div_1, true);
	...
	each(sibling(node, 2), 17, () => get(pagination).items, (job) => job.name, ($$anchor, job) => {
		...
		var consequent_3 = ($$anchor) => {
			var button = root_4$2();
			template_effect(() => {
				set_attribute(button, "aria-label", `Edit ${get(job).name}`);
				button.disabled = get(saving);
			});
			delegated("click", button, () => edit(get(job)));
			append($$anchor, button);
		};
		...
		var text_8 = only_child(button_1, true);
		...
		template_effect(($0) => {
			...
			set_attribute(button_1, "aria-label", `${get(job).paused ? "Resume" : "Pause"} ${get(job).name}`);
			button_1.disabled = get(saving);
			set_text(text_8, get(job).paused ? "Resume" : "Pause");
			set_attribute(button_2, "aria-label", `Remove ${get(job).name}`);
			button_2.disabled = get(saving);
		}, [() => scheduleText(get(job))]);
		delegated("click", button_1, () => void command({
			action: get(job).paused ? "resume" : "pause",
			name: get(job).name
		}, `${get(job).paused ? "Resumed" : "Paused"} ${get(job).name}`));
		delegated("click", button_2, () => requestRemove(get(job)));
	...
	var consequent_4 = ($$anchor) => {
		...
		reset(nav);
		template_effect(() => {
			button_3.disabled = get(pagination).page === 1;
			set_text(text_9, `Page ${get(pagination).page ?? ""} of ${get(pagination).pageCount ?? ""}`);
			button_4.disabled = get(pagination).page === get(pagination).pageCount;
		});
		delegated("click", button_3, () => set(page, get(pagination).page - 1));
	...
	reset(div_12);
	reset(form);
	var button_7 = only_child(sibling(form, 2));
	reset(div);
	var dialog = sibling(div, 2);
	var div_14 = child(dialog);
	var h3_1 = child(div_14);
	var text_12 = only_child(h3_1, true);
	var p = sibling(h3_1, 2);
	var node_8 = child(p);
	var consequent_7 = ($$anchor) => {
		var text_13 = text();
		template_effect(() => set_text(text_13, `This will remove all ${get(confirmation).count ?? ""} configured jobs. This cannot be undone.`));
		append($$anchor, text_13);
	};
	var consequent_8 = ($$anchor) => {
		var text_14 = text();
		template_effect(() => set_text(text_14, `Remove “${get(confirmation).job.name ?? ""}”? This cannot be undone.`));
		append($$anchor, text_14);
	};
	if_block(node_8, ($$render) => {
		if (get(confirmation)?.kind === "clear") $$render(consequent_7);
		else if (get(confirmation)?.kind === "remove") $$render(consequent_8, 1);
	});
	reset(p);
	...
	reset(div_14);
	reset(dialog);
	attach(dialog, () => dialogVisibility(() => Boolean(get(confirmation)), () => true, cancelConfirmation));
	reset(section);
	template_effect(($0) => {
		set_text(text$1, get(status));
		set_text(text_10, get(editing) ? `Edit ${get(editing)}` : "Add job");
		input.readOnly = $0;
		button_5.disabled = get(saving);
		set_text(text_11, get(editing) ? "Cancel edit" : "Reset");
		button_6.disabled = get(saving);
		button_7.disabled = !get(jobs).length || get(saving);
		set_text(text_12, get(confirmation)?.kind === "clear" ? "Clear scheduled jobs?" : "Remove scheduled job?");
		button_8.disabled = get(saving);
		button_9.disabled = get(saving);
		set_text(text_15, get(confirmation)?.kind === "clear" ? "Clear all jobs" : "Remove job");
	}, [() => Boolean(get(editing))]);
	event("submit", form, (event) => {
		event.preventDefault();
		submit();
	...
	delegated("click", button_5, reset$1);
	delegated("click", button_7, requestClear);
	delegated("click", dialog, (event) => {
		if (event.target === event.currentTarget) cancelConfirmation();
	});
	delegated("click", button_8, cancelConfirmation);
	delegated("click", button_9, () => void confirmDestructiveAction());
	append($$anchor, section);
	pop();
}
delegate(["click"]);
...
var root$1 = /* @__PURE__ */ from_html(`<span>No cached chats match your search.</span> <button type="button" class="list-empty-action">Clear search</button>`, 1);
var root_1$1 = /* @__PURE__ */ from_html(`<span>No conversations match these filters.</span> <button type="button" class="list-empty-action">Clear filters</button>`, 1);
var root_2$1 = /* @__PURE__ */ from_html(`No cached conversations yet.<br/>Prompta runs will appear here live.`, 1);
...
function SidebarList($$anchor, $$props) {
	...
	var consequent_2 = ($$anchor) => {
		...
		var consequent = ($$anchor) => {
			...
			delegated("click", sibling(first_child(fragment_1), 2), clearSearch);
			append($$anchor, fragment_1);
		};
		var consequent_1 = ($$anchor) => {
			var fragment_2 = root_1$1();
			delegated("click", sibling(first_child(fragment_2), 2), clearFilters);
	...
	var alternate_1 = ($$anchor) => {
		var fragment_4 = comment();
		each(first_child(fragment_4), 17, () => sidebarListState.model.groups, (group) => group.label, ($$anchor, group) => {
			var section = root_7();
			...
			each(sibling(div_1, 2), 17, () => get(group).chats, (chat) => chat.id, ($$anchor, chat) => {
				...
				template_effect(($0) => {
					...
					set_attribute(button_2, "data-chat-id", get(chat).id);
					set_attribute(button_2, "data-optimistic-new", get(chat).optimisticNew ? "true" : "false");
					set_attribute(button_2, "aria-current", get(chat).id === sidebarListState.selectedConversationId ? "true" : void 0);
					set_text(text_1, get(chat).title);
					set_text(text_2, get(chat).preview);
					set_text(text_3, get(chat).jobLabel);
					set_attribute(span_4, "data-activity-at", get(chat).activityAt);
					set_text(text_4, $0);
					set_class(button_3, 1, clsx(["chat-row-pin", { active: get(chat).pinned }]));
					set_attribute(button_3, "data-pin-chat-id", get(chat).id);
					set_attribute(button_3, "aria-label", get(chat).pinned ? "Unpin chat" : "Pin chat");
					set_attribute(button_3, "title", get(chat).pinned ? "Unpin chat" : "Pin chat");
					set_attribute(button_3, "aria-pressed", get(chat).pinned);
				}, [() => formatRelativeTime(get(chat).activityAt, appViewState.clockTick)]);
				delegated("pointerdown", button_2, (event) => startLongPress(event, get(chat).id, get(chat).pinned));
				delegated("pointermove", button_2, moveLongPress);
				delegated("pointerup", button_2, (event) => endLongPress(event, get(chat).id, get(chat).optimisticNew));
				event("pointercancel", button_2, cancelLongPress);
				delegated("contextmenu", button_2, (event) => {
					if (!coarsePointer.current) return;
					event.preventDefault();
					suppressSelectChatId = get(chat).id;
					openActions(get(chat).id, get(chat).pinned);
				});
				delegated("click", button_2, () => selectChat(get(chat).id, get(chat).optimisticNew));
				delegated("click", button_3, () => sidebarListActions.onPin(get(chat).id));
				append($$anchor, div_2);
			});
			reset(section);
			template_effect(() => {
				set_attribute(section, "data-dom-key", "group:" + get(group).label);
				set_text(text, get(group).label);
			});
			append($$anchor, section);
		});
		append($$anchor, fragment_4);
	...
	var consequent_5 = ($$anchor) => {
		...
		template_effect(() => {
			button_4.disabled = sidebarListState.model.loadingMore;
			set_attribute(button_4, "aria-busy", sidebarListState.model.loadingMore);
			set_text(text_5, sidebarListState.model.loadingMore ? "Loading older chats…" : "Load older chats");
		});
		delegated("click", button_4, () => sidebarListActions.onLoadMore());
		append($$anchor, button_4);
	};
	if_block(node_6, ($$render) => {
	...
	var text_6 = only_child(sibling(child(button_5), 2), true);
	reset(button_5);
	var button_6 = sibling(button_5, 2);
	var button_7 = sibling(button_6, 2);
	reset(div_6);
	...
	delegated("click", button_5, togglePinFromActions);
	delegated("click", button_6, markAllReadFromActions);
	delegated("click", button_7, closeActions);
	append($$anchor, fragment);
	pop();
}
...
function App($$anchor, $$props) {
	...
	const sidebarHiddenFromAccessibility = /* @__PURE__ */ user_derived(() => mobileSidebarMedia.current && !sidebarState.open && !sidebarDrag.active);
	...
	each(div_7, 21, () => sidebarFilterOptions, (filter) => filter.key, ($$anchor, filter) => {
		var button_3 = root_3();
		var text_1 = only_child(button_3, true);
		template_effect(() => {
			set_class(button_3, 1, clsx(["sidebar-filter-chip", { active: appViewState.sidebarFilters[get(filter).key] }]));
			set_attribute(button_3, "aria-pressed", appViewState.sidebarFilters[get(filter).key]);
			set_text(text_1, get(filter).label);
		});
		delegated("click", button_3, () => appActions.onSidebarFilter(get(filter).key));
		append($$anchor, button_3);
	});
	reset(div_7);
	reset(div_6);
	reset(div_5);
	var nav = sibling(div_5, 2);
	SidebarList(child(nav), {});
	reset(nav);
	reset(div_4);
	attach(div_4, () => scrollToTopOnRequest(() => appViewState.sidebarTopRequest));
	var div_8 = sibling(div_4, 2);
	var div_9 = child(div_8);
	var text_2 = only_child(sibling(child(div_9), 2), true);
	reset(div_9);
	...
	var div_10 = sibling(aside, 2);
	let styles_1;
	var main = sibling(div_10, 2);
	var header = child(main);
	var button_5 = child(header);
	attach(button_5, () => focusOnRequest(() => sidebarState.openerFocusRequest));
	...
	var button_8 = sibling(div_16, 2);
	var text_7 = only_child(button_8, true);
	ChangelogDialog(sibling(button_8, 2), {});
	template_effect(($0) => {
		set_class(aside, 1, clsx(["sidebar", { "is-open": sidebarState.open }]));
		aside.inert = get(sidebarHiddenFromAccessibility);
		set_attribute(aside, "aria-hidden", get(sidebarHiddenFromAccessibility) ? "true" : void 0);
		...
		set_class(button_1, 1, clsx(["brand-home-button", { active: appViewState.mode === "prompta" }]));
		set_attribute(button_1, "aria-current", appViewState.mode === "prompta" ? "page" : void 0);
		set_text(text, get(serverLabel));
		set_class(div_3, 1, clsx(["live-orb", { live: appViewState.live }]));
		set_attribute(div_3, "aria-label", appViewState.liveTitle);
		set_attribute(div_3, "title", appViewState.liveTitle);
		...
		set_class(div_10, 1, clsx(["sidebar-scrim", { "is-open": sidebarState.open }]));
		styles_1 = set_style(div_10, "", styles_1, {
			opacity: $0,
		...
		set_attribute(div_15, "hidden", !appViewState.emptyVisible);
	...
	delegated("pointerup", div_10, () => closeSidebar());
...
function createCompletionNotifications({ displayServerName, getServerName, chatTitle }) {
	...
	function completionTitle(chat, display) {
		...
		const appTitles = /* @__PURE__ */ new Set(["prompta", `prompta · ${String(display || "").trim().toLowerCase()}`]);
		const genericAppTitle = (value) => appTitles.has(String(value || "").trim().toLowerCase());
		...
		for (const value of [
			...
			if (fallback && fallback.toLowerCase() !== "new chat" && !genericAppTitle(fallback)) return fallback.slice(0, 72);
		}
...
function showMode(mode) {
	...
	appViewState.mode = state.mode === "logs" ? "logs" : state.mode === "prompta" ? "prompta" : "chats";
...
async function sendSelectedMessage() {
	...
	if (!composerHasContent(message, attachments.length) || state.mode !== "chats" || state.sending) return;
...
var init_app = __esmMin((() => {
	...
	appActions.onClearSidebarFilters = () => {
		for (const filter of Object.keys(appViewState.sidebarFilters)) appViewState.sidebarFilters[filter] = false;
		state.sidebarFingerprint = "";
		renderSidebar(true);
	};
	appActions.onMarkAllRead = () => {
